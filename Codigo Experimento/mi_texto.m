...
[numtexto,ok] = listdlg('PromptString','Elija un texto a presentar:',...
...
try       
    ...
    answer = inputdlg('Ingrese solo iniciales del sujeto','Nombre',1,{'test'});
    %answer={'test'};
    if isempty(answer)
    ...
    if USOEYETRACKER
        ...
        transfiere_imagen_promedio(init,pantallas);        
        sca
        WaitSecs(1);
    end
...
try
    ...
    if init.width~=1024
        sca
        disp('Hay que correr en 1024x768')
        msgbox('Hay que correr en 1024x768')        
        ShowCursor;%muestra el cursor
        ListenChar(0)%vuelve a la normalidad. si no pude ejecutarlo, ctrl+c hace lo mismo
        error('Hay que correr en 1024x768')
    end
catch ME
    Screen('CloseAll')
    ShowCursor;%muestra el cursor
